...
/** \brief filter llama a pFunc y le pasa cada uno de los elementos de la lista y si pFunc devuelve 1
 *           a ese elemento lo carga en la nueva lista, y sino no.
 *
 * \param
 * \param
 * \return Devuelve un nuevo LinkedList que tiene
 *
 */
...
/** \brief map llama a pFunc y le pasa cada uno de los elementos de la lista y si pFunc devuelve 1
 *           a ese elemento lo carga en la nueva lista, y sino no.
 *
 * \param
 * \param
...
{
    LinkedList* listaAsignada = NULL;
    void* auxElement = NULL;

    if(this != NULL && pFunc != NULL)
    {
        listaAsignada = ll_newLinkedList();
        if(listaAsignada != NULL)
        {
            for(int i=0; i<ll_len(this); i++)
            {
                ...
                if( pFunc(auxElement) )
                {
                    ll_add(listaAsignada, auxElement);
                }
            }
        }
    }

    return listaAsignada;
}
